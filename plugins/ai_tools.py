...
def simple_summarize(text: str, max_sentences: int = 3) -> str:
    ...
    words = re.findall(r"\\b[\\w\\u0621-\\u064A]+\\b", text.lower())
    freq = {}
    for w in words:
    ...
    scores = []
    for s in sentences:
        sw = re.findall(r"\\b[\\w\\u0621-\\u064A]+\\b", s.lower())
...
# --- Advanced AI reply (.ذكاء) using Gemini if available, fallback to simple rules ---
...
async def ai_reply_gemini(prompt: str) -> str:
    text = await core_ai.generate(
        f"أجب بإيجاز وبنفس لغة المستخدم عن التالي:\\n{prompt}",
        max_tokens=180,
        temperature=0.6,
    )
    return text or ai_reply_simple(prompt)
...
async def ai_cmd(event):
    ...
    if not msg:
        ...
        return
    answer = await ai_reply_gemini(msg)
...
async def guess_anime_gemini(description: str):
    prompt = (
        ...
        f"الوصف: {description}\\n"
        "أعطِ الإجابة بصيغة نقاط مثل:\\n- هجوم العمالقة (Attack on Titan): سبب قصير"
    )
    return await core_ai.generate(prompt, max_tokens=220, temperature=0.4)
...
async def anime_search_cmd(event):
    ...
    # Try Gemini for broader matching if available
    ai_result = await guess_anime_gemini(desc)
    if ai_result:
