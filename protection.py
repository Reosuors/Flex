...
protection_data = _load_json(
    PROTECTION_FILE,
    {
        "enabled": False,             # حماية الخاص الشاملة (معطلة افتراضياً)
...
@client.on(events.NewMessage(pattern=r"^\.(?:تفعيل_الحماية|تشغيل حماية الخاص)$"))
async def enable_private_protection(event):
    protection_data["enabled"] = True
    save_protection()
    await event.edit("**⎙ تم تشغيل حماية الخاص.**")

@client.on(events.NewMessage(pattern=r"^\.(?:تعطيل_الحماية|ايقاف حماية الخاص)$"))
...
async def handle_private_messages(event):
    # لا تعمل التحذيرات إلا إذا كانت الحماية الشاملة مفعلة ونظام التحذيرات مفعل
    if not event.is_private or not protection_data.get("enabled") or not warnings_data.get("warnings_enabled", False):
        return
