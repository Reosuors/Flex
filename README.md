...
- ذكاء اصطناعي عبر Groq (Llama) مع دعم بديل لـ Gemini عند توفره
...
- `GROQ_API_KEY` مفتاح Groq لتفعيل الذكاء الاصطناعي (المزوّد الأساسي)
- (اختياري) `GEMINI_API_KEY` مفتاح Google Generative AI كبديل عند عدم وجود Groq
...
- `.ذكاء` أو `.ai` - محادثة ذكاء اصطناعي (Groq أولًا ثم Gemini إن توفر)
